...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            foreach (InputAction other in action.actionMap.actions)
            {
                ...
                for (int i = 0; i < other.bindings.Count; i++)
                {
                    ...
                    {
                        other.ApplyBindingOverride(i, oldOverridePath);
                    }
                }
            }
        }
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDupeBinds(action, bindingIndex, allCompositeParts))
                        {
                            action.RemoveBindingOverride(bindingIndex);
                            CleanUp();
        ...
        private bool CheckDupeBinds(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            InputBinding newBinding = action.bindings[bindingIndex];
        ...
        [Tooltip("Optional bool field. Allows for personal text override on action label.")]
        public bool m_OverrideActionLabel;

        [Tooltip("Action Label Overridden text")]
        [SerializeField]
